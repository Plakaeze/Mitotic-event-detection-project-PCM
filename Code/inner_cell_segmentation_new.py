# -*- coding: utf-8 -*-
"""
Created on Mon Sep 19 16:14:58 2022
...
for s in tqdm(range(1,5)):
    ...
    for p in range(len(full_mito_path)):
        ...
        contours = measure.find_contours(marker)
        ...
        # np.save(path_save_mitosis_im + pre_vid + str(p),cropped_im)
        # np.save(path_save_mitosis_marker + pre_vid + str(p),cropped_marker)
    ...
    column_features = ['Area of mother cell','Major axis of mother cell','Minor axis of mother cell',
                        ...
                        'Distance between 2 cells','Distance of mother cell to celtroid','Distance of daughter cell to celtroid','Distance ratio',
                        'Percentage coverage cell 1','Percentage coverage cell 2','Average coverage area',
                        'Distance factor','Intensity factor','Circularity factor',
                        'Path length','Range of area change in phenomenon','STD of area change in phenomenon','Range of circularity change in phenomenon',
                        'STD of circularity change in phenomenon','Range of extent change in phenomenon','STD of extent change in phenomenon',
                        'Range of mean intensity change in phenomenon','STD of mean intensity change in phenomenon',
                        'Range of minor axis change in phenomenon','STD of minor axis change in phenomenon',
                        'Range of major axis change in phenomenon','STD of major axis change in phenomenon']
    
    features = pd.DataFrame(columns = column_features)
    path_save_segment = 'C:\\Mitotic Event Detection\\Results\\Segmentation\\'
    print('Save images process')
    for fmp in tqdm(range(len(full_mito_path))):
        test_path = full_mito_path[fmp]
        start_frame = test_path[0]
        stop_frame = test_path[-1]
        path = test_path[1:-1]
        path_ind = 0
        path_length = stop_frame - start_frame + 1
        ...
        for f in range(start_frame,stop_frame+1):
            ...
            props_cim = measure.regionprops_table(lab_cim,cropped_im,properties = 
                                                                  ...
                                                                  'axis_major_length','axis_minor_length',
                                                                  'eccentricity','intensity_min',
                                                                  'intensity_max','coords','centroid','extent'])
            cim_phe_c0 = props_cim['centroid-0']
            cim_phe_c1 = props_cim['centroid-1']
            ...
            phe_area_ecc.append(1 - props_cim['eccentricity'][np.argmin(cim_eu)])
            mu_inten_path.append(props_cim['intensity_mean'][np.argmin(cim_eu)])
            phe_mi.append(1 - props_cim['axis_minor_length'][np.argmin(cim_eu)])
            phe_maj.append(1 - props_cim['axis_major_length'][np.argmin(cim_eu)])
            phe_area.append(props_cim['area'][np.argmin(cim_eu)])
            phe_ext.append(props_cim['extent'][np.argmin(cim_eu)])
            max_inten_path.append(np.max(cropped_im))
        ...
        idx_mn4 = np.argmin(mito_eu)
        mito_eu[idx_mn4] = np.Inf
        idx_mn5 = np.argmin(mito_eu)
        mito_eu[idx_mn5] = np.Inf
        
        closest_point = np.asarray([maped_mito_coordinates[idx_mn1,:],
                        maped_mito_coordinates[idx_mn2,:],maped_mito_coordinates[idx_mn3,:],
                        maped_mito_coordinates[idx_mn4,:],maped_mito_coordinates[idx_mn5,:]])
        ...
        scalebar = ScaleBar(1.5, "um", length_fraction=0.20, location='lower right',
                            font_properties={"size": 25})
        
        imgo = ax5[0].imshow(cropped)
        ...
        ax5[0].add_artist(scalebar)
        ax5[0].get_xaxis().set_visible(False)
        ax5[0].get_yaxis().set_visible(False)
        divider2 = make_axes_locatable(ax5[0])
        cax2 = divider2.append_axes("right", size="5%", pad=0.1)
        fig5.colorbar(imgo,cax = cax2)
        cax2.tick_params(labelsize=25)
        
        ws = ax5[1].imshow(watershed_tf)
        ...
        scalebar = ScaleBar(1.5, "um", length_fraction=0.20, location='lower right',
                            font_properties={"size": 25})
        ax5[1].add_artist(scalebar)
        ax5[1].get_xaxis().set_visible(False)
        ax5[1].get_yaxis().set_visible(False)
        divider = make_axes_locatable(ax5[1])
        cax = divider.append_axes("right", size="5%", pad=0.1)
        fig5.colorbar(ws,cax = cax)
        ...
        # fig6, ax6 = try_all_threshold(cropped, figsize=(10, 8), verbose=False)
        # plt.show()
        ...
        # Distance features
        c0m = m_props['centroid-0'].to_numpy()[0]
        c1m = m_props['centroid-1'].to_numpy()[0]
        c0d = d_props['centroid-0'].to_numpy()[0]
        c1d = d_props['centroid-1'].to_numpy()[0]
        
        eu_bet = np.sqrt(np.power(c0m - c0d,2)+np.power(c1m - c1d,2))
        eu_m_phe = np.sqrt(np.power(c0m - mapped_mito_area_c0,2)
                            +np.power(c1m - mapped_mito_area_c1,2))
        eu_d_phe = np.sqrt(np.power(c0d - mapped_mito_area_c0,2)
                            +np.power(c1d - mapped_mito_area_c1,2))
        eu_ratio = eu_m_phe/eu_d_phe
        
        # Coverage phenomenon area to detected cell
        m_coord = m_props['coords'].to_numpy()[0]
        d_coord = d_props['coords'].to_numpy()[0]
        
        if not isinstance(m_coord, float):
        ...
        if not isinstance(m_cov0, int):
            ...
            avg_cov = (percentage_cov_m+percentage_cov_d)/2
        else:
            percentage_cov_m = 0
            percentage_cov_d = 0
            avg_cov = (percentage_cov_m+percentage_cov_d)/2
        ...
        # Factors
        distance_factors_f = distance_factors[np.argmax(mitosis_factor)]
        intensity_factor_f = intensity_factor[np.argmax(mitosis_factor)]
        ecc_factors_f = ecc_factors[np.argmax(mitosis_factor)]
        
        # sequence based features
        arr_phe_area = np.asarray(phe_area)
        range_area = np.max(arr_phe_area) - np.min(arr_phe_area)
        std_area = np.std(arr_phe_area)
        
        range_ecc = np.max(phe_area_ecc) - np.min(phe_area_ecc)
        std_ecc = np.std(phe_area_ecc)
        
        arr_phe_extent = np.asarray(phe_ext)
        range_ext_phe = np.max(arr_phe_extent) - np.min(arr_phe_extent)
        std_ext_phe = np.std(arr_phe_extent)
        
        range_mint_phe = np.max(arr_mean_white_area) - np.min(arr_mean_white_area)
        std_mint_phe = np.std(arr_mean_white_area)
        ...
        features_list = [area_m,maj_m,mi_m,cir_m,mean_int_m,max_int_m,min_int_m,axis_ratio_m,
                    ...
                    eu_bet,eu_m_phe,eu_d_phe,eu_ratio,
                    percentage_cov_m,percentage_cov_d,avg_cov,
                    distance_factors_f,intensity_factor_f,ecc_factors_f,
                    path_length,range_area,std_area,range_ecc,std_ecc,
                    range_ext_phe,std_ext_phe,range_mint_phe,std_mint_phe,
                    range_phe_mi,std_phe_mi,range_phe_maj,std_phe_maj]
    ...
    # path_save_features = 'C:\\Mitotic Event Detection\\New features\\' + seq +'\\'
    # path_save_features = 'C:\\Mitotic Event Detection\\New features longer\\' + seq +'\\'
